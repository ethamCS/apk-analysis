...
final class MethodGenericTypeResolver {
    static TypeBindings bindMethodTypeParameters(Method method, JavaType javaType, TypeResolutionContext typeResolutionContext) {
        ...
        for (int i10 = 0; i10 < actualTypeArguments.length; i10++) {
            TypeVariable<?> maybeGetTypeVariable = maybeGetTypeVariable(actualTypeArguments[i10]);
            if (maybeGetTypeVariable != null) {
                String name = maybeGetTypeVariable.getName();
                if (name == null || (boundType = javaType.getBindings().getBoundType(i10)) == null || (findByName = findByName(typeParameters, name)) == null) {
                    return null;
                }
    ...
    private static boolean pessimisticallyValidateBound(TypeResolutionContext typeResolutionContext, JavaType javaType, Type type) {
        ...
        for (int i10 = 0; i10 < bindings.size(); i10++) {
            if (!pessimisticallyValidateBound(typeResolutionContext, bindings.getBoundType(i10), actualTypeArguments[i10])) {
                return false;
            }
        }
        return true;
