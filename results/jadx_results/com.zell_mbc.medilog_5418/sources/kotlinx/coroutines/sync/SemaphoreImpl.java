            ...
            while (true) {
                if (semaphoreSegment2.getId() < j || semaphoreSegment2.getRemoved()) {
                    ...
                    if (nextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                        obj = SegmentOrClosed.m6269constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    obj = SegmentOrClosed.m6269constructorimpl(semaphoreSegment2);
                    break;
                }
            }
            z = true;
            if (SegmentOrClosed.m6274isClosedimpl(obj)) {
                break;
            }
            Segment m6272getSegmentimpl = SegmentOrClosed.m6272getSegmentimpl(obj);
            while (true) {
                Segment segment = (Segment) this.tail;
                if (segment.getId() >= m6272getSegmentimpl.getId()) {
                    break;
                } else if (!m6272getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z2 = false;
                    continue;
                    break;
                } else if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment, m6272getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                } else if (m6272getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m6272getSegmentimpl.remove();
                }
            }
            z2 = true;
            continue;
        } while (!z2);
        SemaphoreSegment semaphoreSegment4 = (SemaphoreSegment) SegmentOrClosed.m6272getSegmentimpl(obj);
