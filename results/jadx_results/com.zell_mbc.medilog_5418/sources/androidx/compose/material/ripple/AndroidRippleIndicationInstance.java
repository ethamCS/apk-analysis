...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.Companion.m1537getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo2115getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1308getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo2115getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo325roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m1706unboximpl = this.color.getValue().m1706unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1314drawStateLayerH2RKhps(contentDrawScope2, this.radius, m1706unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1313updateRipplePropertiesbiQXAtU(contentDrawScope.mo2115getSizeNHjbRc(), this.rippleRadius, m1706unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1312addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m1706unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
