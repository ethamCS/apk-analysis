...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m3667getMinimpl(j);
        this.selectionEnd = TextRange.m3666getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m3667getMinimpl = TextRange.m3667getMinimpl(j);
        int m3666getMaximpl = TextRange.m3666getMaximpl(j);
        if (m3667getMinimpl < 0 || m3667getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m3667getMinimpl + ") offset is outside of text region " + annotatedString.length());
        } else if (m3666getMaximpl < 0 || m3666getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m3666getMaximpl + ") offset is outside of text region " + annotatedString.length());
        } else if (m3667getMinimpl <= m3666getMaximpl) {
        } else {
            throw new IllegalArgumentException("Do not set reversed range: " + m3667getMinimpl + " > " + m3666getMaximpl);
        }
    }
    ...
    public final TextRange m3799getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m3657boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release */
    public final long m3800getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int i, int i2) {
        ...
        long m3801updateRangeAfterDeletepWDy79M = EditingBufferKt.m3801updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m3667getMinimpl(m3801updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m3666getMaximpl(m3801updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m3801updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m3801updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m3663getCollapsedimpl(m3801updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
                return;
            }
            this.compositionStart = TextRange.m3667getMinimpl(m3801updateRangeAfterDeletepWDy79M2);
            this.compositionEnd = TextRange.m3666getMaximpl(m3801updateRangeAfterDeletepWDy79M2);
        }
    }
