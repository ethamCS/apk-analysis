...
public final class AndroidCanvas implements Canvas {
    ...
    public /* synthetic */ void mo1555clipRectmtrdDE(androidx.compose.ui.geometry.Rect rect, int i) {
        Canvas.CC.m1666$default$clipRectmtrdDE(this, rect, i);
    }
    ...
    public void mo1556concat58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        if (!MatrixKt.m1918isIdentity58bKbWc(matrix)) {
            android.graphics.Matrix matrix2 = new android.graphics.Matrix();
            AndroidMatrixConversions_androidKt.m1571setFromEL8BTi8(matrix2, matrix);
    ...
    public void mo1554clipRectN_I0leg(float f, float f2, float f3, float f4, int i) {
        this.internalCanvas.clipRect(f, f2, f3, f4, m1564toRegionOp7u2Bmg(i));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E */
    public void mo1553clipPathmtrdDE(Path path, int i) {
        Intrinsics.checkNotNullParameter(path, "path");
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m1564toRegionOp7u2Bmg(i));
            return;
        }
        throw new UnsupportedOperationException("Unable to obtain android.graphics.Path");
    }

    /* renamed from: toRegionOp--7u2Bmg */
    public final Region.Op m1564toRegionOp7u2Bmg(int i) {
        return ClipOp.m1680equalsimpl0(i, ClipOp.Companion.m1684getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g */
    public void mo1560drawLineWko1d7g(long j, long j2, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawLine(Offset.m1459getXimpl(j), Offset.m1460getYimpl(j), Offset.m1459getXimpl(j2), Offset.m1460getYimpl(j2), paint.asFrameworkPaint());
    }
    ...
    public void mo1557drawCircle9KIMszo(long j, float f, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawCircle(Offset.m1459getXimpl(j), Offset.m1460getYimpl(j), f, paint.asFrameworkPaint());
    }
    ...
    public void mo1558drawImaged4ec7I(ImageBitmap image, long j, Paint paint) {
        Intrinsics.checkNotNullParameter(image, "image");
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m1459getXimpl(j), Offset.m1460getYimpl(j), paint.asFrameworkPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0 */
    public void mo1559drawImageRectHPBpro0(ImageBitmap image, long j, long j2, long j3, long j4, Paint paint) {
        Intrinsics.checkNotNullParameter(image, "image");
        Intrinsics.checkNotNullParameter(paint, "paint");
        android.graphics.Canvas canvas = this.internalCanvas;
        Bitmap asAndroidBitmap = AndroidImageBitmap_androidKt.asAndroidBitmap(image);
        Rect rect = this.srcRect;
        rect.left = IntOffset.m4224getXimpl(j);
        rect.top = IntOffset.m4225getYimpl(j);
        rect.right = IntOffset.m4224getXimpl(j) + IntSize.m4266getWidthimpl(j2);
        rect.bottom = IntOffset.m4225getYimpl(j) + IntSize.m4265getHeightimpl(j2);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        rect2.left = IntOffset.m4224getXimpl(j3);
        rect2.top = IntOffset.m4225getYimpl(j3);
        rect2.right = IntOffset.m4224getXimpl(j3) + IntSize.m4266getWidthimpl(j4);
        rect2.bottom = IntOffset.m4225getYimpl(j3) + IntSize.m4265getHeightimpl(j4);
    ...
    public void mo1561drawPointsO7TthRY(int i, List<Offset> points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
        if (PointMode.m1961equalsimpl0(i, PointMode.Companion.m1965getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m1961equalsimpl0(i, PointMode.Companion.m1967getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (!PointMode.m1961equalsimpl0(i, PointMode.Companion.m1966getPointsr_lszbg())) {
        } else {
    ...
    private final void drawLines(List<Offset> list, Paint paint, int i) {
        if (list.size() >= 2) {
            ...
            while (true) {
                long m1469unboximpl = list.get(first).m1469unboximpl();
                long m1469unboximpl2 = list.get(first + 1).m1469unboximpl();
                this.internalCanvas.drawLine(Offset.m1459getXimpl(m1469unboximpl), Offset.m1460getYimpl(m1469unboximpl), Offset.m1459getXimpl(m1469unboximpl2), Offset.m1460getYimpl(m1469unboximpl2), paint.asFrameworkPaint());
                if (first == last) {
                    return;
                }
                first += step2;
    ...
    public void mo1562drawRawPointsO7TthRY(int i, float[] points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
