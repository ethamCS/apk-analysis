...
public final class TrieNode<K, V> {
    ...
    private final TrieNode<K, V> mutableCollisionPutAll(TrieNode<K, V> trieNode, DeltaCounter deltaCounter, MutabilityOwnership mutabilityOwnership) {
        CommonFunctionsKt.m1351assert(this.nodeMap == 0);
        CommonFunctionsKt.m1351assert(this.dataMap == 0);
        CommonFunctionsKt.m1351assert(trieNode.nodeMap == 0);
        CommonFunctionsKt.m1351assert(trieNode.dataMap == 0);
        Object[] objArr = this.buffer;
    ...
    private final TrieNode<K, V> mutablePutAllFromOtherNodeCell(TrieNode<K, V> trieNode, int i, int i2, DeltaCounter deltaCounter, PersistentHashMapBuilder<K, V> persistentHashMapBuilder) {
        ...
        if (hasNodeAt(i)) {
            ...
            if (hasEntryAt$runtime_release(i)) {
                ...
                int i4 = i2 + 5;
                if (nodeAtIndex$runtime_release2.containsKey(keyAtIndex2 != null ? keyAtIndex2.hashCode() : 0, keyAtIndex2, i4)) {
                    deltaCounter.setCount(deltaCounter.getCount() + 1);
                } else {
                    return nodeAtIndex$runtime_release2.mutablePut(keyAtIndex2 != null ? keyAtIndex2.hashCode() : 0, keyAtIndex2, valueAtKeyIndex(entryKeyIndex$runtime_release2), i4, persistentHashMapBuilder);
                }
            }
            ...
            V valueAtKeyIndex3 = trieNode.valueAtKeyIndex(entryKeyIndex$runtime_release4);
            int hashCode = keyAtIndex3 != null ? keyAtIndex3.hashCode() : 0;
            if (keyAtIndex4 != null) {
                i3 = keyAtIndex4.hashCode();
            }
            return makeNode(hashCode, keyAtIndex3, valueAtKeyIndex2, i3, keyAtIndex4, valueAtKeyIndex3, i2 + 5, persistentHashMapBuilder.getOwnership$runtime_release());
        }
    }
    ...
    public final TrieNode<K, V> mutablePut(int i, K k, V v, int i2, PersistentHashMapBuilder<K, V> mutator) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            ...
            TrieNode<K, V> nodeAtIndex$runtime_release = nodeAtIndex$runtime_release(nodeIndex$runtime_release);
            if (i2 == 30) {
                trieNode = nodeAtIndex$runtime_release.mutableCollisionPut(k, v, mutator);
            } else {
                trieNode = nodeAtIndex$runtime_release.mutablePut(i, k, v, i2 + 5, mutator);
            }
            return nodeAtIndex$runtime_release == trieNode ? this : mutableUpdateNodeAtIndex(nodeIndex$runtime_release, trieNode, mutator.getOwnership$runtime_release());
        } else {
            mutator.setSize(mutator.size() + 1);
    ...
    public final TrieNode<K, V> mutableRemove(int i, K k, int i2, PersistentHashMapBuilder<K, V> mutator) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            ...
            TrieNode<K, V> nodeAtIndex$runtime_release = nodeAtIndex$runtime_release(nodeIndex$runtime_release);
            if (i2 == 30) {
                trieNode = nodeAtIndex$runtime_release.mutableCollisionRemove(k, mutator);
            } else {
                trieNode = nodeAtIndex$runtime_release.mutableRemove(i, k, i2 + 5, mutator);
            }
            return mutableReplaceNode(nodeAtIndex$runtime_release, trieNode, nodeIndex$runtime_release, indexSegment, mutator.getOwnership$runtime_release());
        }
    }
    ...
    public final TrieNode<K, V> remove(int i, K k, V v, int i2) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            int entryKeyIndex$runtime_release = entryKeyIndex$runtime_release(indexSegment);
            return (!Intrinsics.areEqual(k, keyAtIndex(entryKeyIndex$runtime_release)) || !Intrinsics.areEqual(v, valueAtKeyIndex(entryKeyIndex$runtime_release))) ? this : removeEntryAtIndex(entryKeyIndex$runtime_release, indexSegment);
        } else if (!hasNodeAt(indexSegment)) {
            return this;
        } else {
            int nodeIndex$runtime_release = nodeIndex$runtime_release(indexSegment);
    ...
    public final TrieNode<K, V> mutableRemove(int i, K k, V v, int i2, PersistentHashMapBuilder<K, V> mutator) {
        ...
        if (hasEntryAt$runtime_release(indexSegment)) {
            int entryKeyIndex$runtime_release = entryKeyIndex$runtime_release(indexSegment);
            return (!Intrinsics.areEqual(k, keyAtIndex(entryKeyIndex$runtime_release)) || !Intrinsics.areEqual(v, valueAtKeyIndex(entryKeyIndex$runtime_release))) ? this : mutableRemoveEntryAtIndex(entryKeyIndex$runtime_release, indexSegment, mutator);
        } else if (!hasNodeAt(indexSegment)) {
            return this;
        } else {
            int nodeIndex$runtime_release = nodeIndex$runtime_release(indexSegment);
            TrieNode<K, V> nodeAtIndex$runtime_release = nodeAtIndex$runtime_release(nodeIndex$runtime_release);
            if (i2 == 30) {
                trieNode = nodeAtIndex$runtime_release.mutableCollisionRemove(k, v, mutator);
            } else {
                trieNode = nodeAtIndex$runtime_release.mutableRemove(i, k, v, i2 + 5, mutator);
            }
            return mutableReplaceNode(nodeAtIndex$runtime_release, trieNode, nodeIndex$runtime_release, indexSegment, mutator.getOwnership$runtime_release());
        }
    }
