    ...
    public void setValue(float[] fArr) {
        ...
        switch (this.mType) {
            ...
            case TypedValues.Custom.TYPE_COLOR /* 902 */:
                ...
                float f2 = fArr[1];
                float f3 = fArr[2];
                this.mIntegerValue = ((Math.round(fArr[3] * 255.0f) & 255) << 24) | ((Math.round(((float) Math.pow(f, 0.5d)) * 255.0f) & 255) << 16) | ((Math.round(((float) Math.pow(f2, 0.5d)) * 255.0f) & 255) << 8) | (Math.round(((float) Math.pow(f3, 0.5d)) * 255.0f) & 255);
                return;
            case TypedValues.Custom.TYPE_STRING /* 903 */:
                throw new RuntimeException("Cannot interpolate String");
            case TypedValues.Custom.TYPE_BOOLEAN /* 904 */:
