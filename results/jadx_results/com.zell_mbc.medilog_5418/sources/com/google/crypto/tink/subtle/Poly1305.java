...
class Poly1305 {
    ...
    private static long load32(byte[] in, int idx) {
        return (((in[idx + 3] & UByte.MAX_VALUE) << 24) | (in[idx] & UByte.MAX_VALUE) | ((in[idx + 1] & UByte.MAX_VALUE) << 8) | ((in[idx + 2] & UByte.MAX_VALUE) << 16)) & InternalZipConstants.ZIP_64_SIZE_LIMIT;
    }
    ...
    public static byte[] computeMac(final byte[] key, byte[] data) {
        ...
        long load32 = j41 + load32(key, 16);
        long j45 = load32 & InternalZipConstants.ZIP_64_SIZE_LIMIT;
        long load322 = j42 + load32(key, 20) + (load32 >> 32);
        long j46 = load322 & InternalZipConstants.ZIP_64_SIZE_LIMIT;
        long load323 = j43 + load32(key, 24) + (load322 >> 32);
        long j47 = load323 & InternalZipConstants.ZIP_64_SIZE_LIMIT;
        long load324 = (j44 + load32(key, 28) + (load323 >> 32)) & InternalZipConstants.ZIP_64_SIZE_LIMIT;
        byte[] bArr2 = new byte[16];
        toByteArray(bArr2, j45, 0);
        toByteArray(bArr2, j46, 4);
        toByteArray(bArr2, j47, 8);
